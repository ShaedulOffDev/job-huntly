...
import { categories, specialists } from "@/mock-data";
    ...
    <div>
      <div className="bg-[#F8F8FD]">
        <div className="py-14 container">
          ...
          <p className="sm:text-center mt-8 text-lg text-gray-500">Find specialists for your company to make your business stronger</p>
          <div className="mt-10">
      ...
      <div className="py-10 flex gap-8 container">
        <aside
          className={`w-52 text-text overflow-auto max-lg:h-full flex-shrink-0 max-lg:fixed top-0 end-0 z-50 max-lg:w-full max-lg:bg-black max-lg:bg-opacity-50 ${
            !filter && window.innerWidth < 1023.5 && "max-lg:hidden"
          }`}
        >
              ...
              >
                <div
                  className={`flex items-start gap-3 ${
                    jobView == "cube"
                      ? "mb-4 max-lg:flex-col max-lg:text-center max-lg:items-center"
                      : "max-[450px]:flex-col max-[450px]:text-center max-[450px]:items-center"
                  }`}
                >
                  <div className="w-16 overflow-hidden aspect-square rounded-full flex-shrink-0">
                    <Image src={s.image} alt={s.name} className="object-contain max-w-full h-full" />
                  </div>
                  <div>
                    <h3 className={`text-2xl font-semibold ${jobView == "cube" && "max-sm:text-center"}`}>{s.name}</h3>
                    <p className={`text-sm sm:text-base ${jobView == "cube" && "max-sm:text-center"}`}>{s.job}</p>
                    <p className="text-gray-500 mt-1">{s.description.length > 150 ? s.description.slice(0, 150) + '...' : s.description}</p>
                  </div>
                </div>
              </Link>
            ))}
